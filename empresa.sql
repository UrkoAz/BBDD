...
CREATE TABLE ACCESORIOS(
    ...
    DESCRIPCION_ACC VARCHAR(60) NOT NULL,
    MARCA_ACC TEXT NOT NULL,
    TIPO_ACC TEXT NOT NULL,
    ID_DISP INT NOT NULL
);

/*CREAMOS LAS PRIMARY KEYS*/
...
/*AÑADIMOS LA INFORMACIÓN CORRESPONDIENTE A LAS TABLAS MEDIANTE INSERTS*/
...
/*CREAR UNA CONSULTA CON CADA UNA DE FROM, WHERE, GROUP BY, ORDER BY, HAVING. MÁXIMO 2 DE CADA UNA*/
SELECT ID_MOVIL, MARCA_MOV, TIPO_MOV FROM MOVIL WHERE ID_MOVIL!=151 ORDER BY ID_MOVIL DESC;
SELECT ID_DEP, NOMBRE_DEP FROM DEPARTAMENTO GROUP BY ID_DEP;
SELECT ID_EMPLEADOS, NOMBRE_EMPL, SALARIO FROM EMPLEADOS GROUP BY ID_EMPLEADOS HAVING MAX(SALARIO)>2000;

/*2 CONSULTAS CON FUNCIONES*/
SELECT ID_EMPLEADOS, NOMBRE_EMPL, MAX(SALARIO) FROM EMPLEADOS GROUP BY ID_EMPLEADOS;
SELECT SUM(SALARIO) FROM EMPLEADOS;

/*2 CONSULTAS USANDO MÁS DE UN OPERADOR LÓGICO*/
SELECT ID_ACCESORIOS, MARCA_ACC,DESCRIPCION_ACC FROM ACCESORIOS WHERE MARCA_ACC="MSI" OR MARCA_ACC="Acer";
SELECT ID_DEP, NOMBRE_DEP, NUM_TRAB FROM DEPARTAMENTO WHERE ID_DEP=78 AND NOMBRE_DEP="Departamento Logistica";

/*2 CONSULTAS CON PREDICADO*/
SELECT ID_DISP, MARCA_DISP FROM DISPOSITIVOS WHERE ID_DISP BETWEEN 3000 AND 4000 ORDER BY ID_DISP DESC;
SELECT NOMBRE_EMPL, ID_EMPLEADOS FROM EMPLEADOS WHERE NOMBRE_EMPL LIKE "M%";
